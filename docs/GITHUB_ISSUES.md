...
- **依赖**：JAI-024
- **状态**：D-037 G1～G4 的实现、双语文档和 350 项 PostgreSQL 完整门禁已通过；远程 feature 末端为 `ff423f1ec3b622b6bb934519f57ccf5d08cac885`。M-001/M-002 已完成并安全验证；业务迁移 `0010` 与一次指定快照真实核验仍等待 `A-001/G5`，因此 Issue 未完成、未合入 `develop`。
- **目标**：通过一个选定通道可靠推送日报。
...
- **状态**：方案 1 的实现、浏览器 design QA、357 项 Python 完整门禁及 M-003 容器镜像构建验证均已通过，对应实现检查点为 `96fe7984d0a46e5f2c94b2bddebee35adcdf1377`；技术验收完成，等待 JAI-027 先行集成后再同步、复验并按序合入 `develop`。
...
JAI-021 至 JAI-026 已按顺序完成并合入 `develop`，当前基线为 JAI-026 非快进合并 `a9e9b643b629e5632015778549917f44bd658586`。JAI-027 已在远程独立 feature 分支 `ff423f1ec3b622b6bb934519f57ccf5d08cac885` 完成 D-037 G1～G4 实施、双语文档和 350 项无跳过 PostgreSQL 门禁，覆盖率 85.37%；M-001/M-002 已完成并安全验证，业务迁移和一次指定快照真实测试仍等待 A-001/G5，不得标记 Issue 完成。JAI-050 实现检查点为 `96fe7984d0a46e5f2c94b2bddebee35adcdf1377`，方案 1 实现、浏览器 design QA、357 项完整门禁及 M-003 容器构建验证均已通过；技术验收完成，但不得先于 JAI-027 合入 `develop`。临时表格加速提案已撤回，只保留未合并审计记录，不进入 Backlog；正式顺序保持 JAI-027 → JAI-050 → JAI-028 → JAI-051 → JAI-029。2026-09-25 Docker 恢复后的只读证据显示 2026-09-16 至 2026-09-25 没有运行行，启动日志也没有显式 misfire 事件；负责人随后批准且仅停止 scheduler，`db`/`api` 保持健康，固定作业和既有台账完整保留。缺失日期不作结果推断或补跑。JAI-051 迁移仍受 U3 审批。禁止 rebase、force push 或历史改写。JAI-041 已吸收到 JAI-021；JAI-049 在 MVP 发布闸门前跟踪真实完整率和来源风险。发布闭环稳定后逐站执行其余 JAI-038～JAI-045；JAI-048 保持独立文档 Issue。动态门户不满足公开访问或条款边界时记录 `blocked`，不使用登录、验证码、Playwright 或规避措施强行接入。

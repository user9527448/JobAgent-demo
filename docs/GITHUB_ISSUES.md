...
- **验收标准**：
  - [x] 同一日报同一通道成功发送后不会重复发送。
  - [x] 临时失败按上限重试，永久失败可查看原因。
  - [x] 日志和数据库不泄露 Token。
...
- **状态**：D-038/U1-R 与方案 1 已于 2026-09-14 获批；允许从当前 JAI-027 末端创建独立堆叠分支 `feature/jai-050-production-ui-foundation`，但不得先于 JAI-027 合入 `develop`。
...
- **验收标准**：
  - [ ] 成对 `docs/DESIGN.md` 已纳入 Git、登记到两份索引并作为 UI 改动必读上下文。
  - [ ] 页面只按后端证据展示状态；缺失运行不得推断为成功、失败或 misfire。
  - [ ] 最新日报、四/五阶段兼容运行记录和投递状态具备加载、空、错误及窄屏状态。
  - [ ] 页面没有运行、补跑或发送动作，键盘导航、焦点、对比度和语义结构通过验收。
  - [ ] 生产构建由 FastAPI 同源提供，刷新已登记前端路由不会返回 404。
  - [ ] 自动化检查与对选定视觉基线的 design QA 通过。
...
JAI-021 至 JAI-026 已按顺序完成并合入 `develop`，当前基线为 JAI-026 非快进合并 `a9e9b643b629e5632015778549917f44bd658586`。JAI-027 已在独立 feature 分支完成 D-037 G1～G5：业务库已安全升级到 `0010_notification_delivery`，快照 2 恰好提交一次；PushPlus 返回受理身份后因 AccessKey 拒绝无法确认最终结果，台账按获批修正保守保存为 `unknown` 且禁止重发。修复后的完整 PostgreSQL 门禁为 350 项无跳过测试、覆盖率 85.53%，Issue 已满足验收并等待非快进合入 `develop`。D-038/U1-R 与方案 1 已于 2026-09-14 获批：独立堆叠 JAI-050 分支交付正式只读“晨间简报”，但不得先于 JAI-027 合入 `develop`。随后按 JAI-027 → JAI-050 → JAI-028 → JAI-051 → JAI-029 执行；JAI-051 迁移仍受 U3 审批。禁止 rebase、force push 或历史改写。JAI-041 已吸收到 JAI-021；JAI-049 在 MVP 发布闸门前跟踪真实完整率和来源风险。发布闭环稳定后逐站执行其余 JAI-038～JAI-045；JAI-048 保持独立文档 Issue。动态门户不满足公开访问或条款边界时记录 `blocked`，不使用登录、验证码、Playwright 或规避措施强行接入。

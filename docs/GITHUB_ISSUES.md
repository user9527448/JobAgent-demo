...
JAI-021 至 JAI-025 已按顺序完成并合入 `develop`。JAI-026 已在独立 feature 分支完成 D-036 与 G1～G3 实施、313 项无跳过 PostgreSQL 门禁和双语文档，当前等待普通推送后的 G4 运行启用与安全合并审核；JAI-027 通知不得提前实现。禁止 rebase、force push 或历史改写。JAI-041 已吸收到 JAI-021；JAI-049 在 MVP 发布闸门前跟踪真实完整率和来源风险。发布闭环稳定后逐站执行其余 JAI-038～JAI-045；JAI-048 保持独立文档 Issue。动态门户不满足公开访问或条款边界时记录 `blocked`，不使用登录、验证码、Playwright 或规避措施强行接入。

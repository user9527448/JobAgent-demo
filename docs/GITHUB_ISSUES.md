...
当前顺序为：完成 JAI-047 双语迁移基线 → JAI-012 运行与重试能力 → 按原路线实施 JAI-013～JAI-021。用户明确批准有边界的并行例外：JAI-021 进入仅自然日验收观测通道时，JAI-022 与其后 JAI-023 可在独立分支推进。集成必须严格按 JAI-021 → `develop`、更新的 `develop` → JAI-022 并完成合并、再次更新的 `develop` → JAI-023 执行；每一步保留双语 WORKLOG、显式解决冲突并重跑完整门禁，绝不通过 rebase 或历史改写规避冲突。发布闭环稳定后逐站执行 JAI-038～JAI-045。JAI-048 作为独立文档 Issue，在任何对应存量文档发生下一次实质修改前完成，不混入功能分支。除这次明确批准的观测/后续开发并行外，个人 WIP 上限仍为一个主要功能 Issue 加一个小型测试/文档 Issue。动态门户若不满足公开访问和条款边界，记录 `blocked` 并继续下一站，不通过登录、验证码、Playwright 或规避技术措施强行完成数量目标。

...
当前顺序为：完成 JAI-047 双语迁移基线 → JAI-012 运行与重试能力 → 按原路线完成 JAI-013～JAI-021，使 MVP 达到 5 个稳定来源 → JAI-022 继续主线；JAI-021 登记的真实完整率差距由 JAI-049 在 MVP 发布闸门前整改 → 发布闭环稳定后逐站执行 JAI-038～JAI-045。JAI-048 作为独立文档 Issue，在任何对应存量文档发生下一次实质修改前完成，不混入功能分支。个人 WIP 上限为 2：最多一个主要功能 Issue 加一个小型测试/文档 Issue。动态门户若不满足公开访问和条款边界，记录 `blocked` 并继续下一站，不通过登录、验证码、Playwright 或规避技术措施强行完成数量目标。

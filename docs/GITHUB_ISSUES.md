...
JAI-021 已按用户明确记录的 Day 3 外部入口豁免完成并先行合入 `develop`。当前按授权的安全合并火车依次处理 JAI-022 → JAI-023 → JAI-024：每个 feature 分支先普通合并最新 `develop`，保留双方双语日志并重跑 PostgreSQL 完整门禁，之后才允许非快进合入 `develop`；禁止 rebase、force push 或历史改写。JAI-041 的中国移动公开公告最小范围已吸收到 JAI-021，不再重复执行；JAI-049 在 MVP 发布闸门前跟踪真实完整率和来源风险。发布闭环稳定后逐站执行其余 JAI-038～JAI-045；JAI-048 保持独立文档 Issue。动态门户不满足公开访问或条款边界时记录 `blocked`，不使用登录、验证码、Playwright 或规避措施强行接入。

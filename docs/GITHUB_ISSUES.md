...
- **范围**：分析 NCSS 缺失截止时间和江苏综合专题缺失单位/截止时间的来源结构；优先选择同一官方主体的稳定细分公告入口、增加有直接引文的确定性规则，或按 W6 闸门替换低价值来源；不得把发布时间当截止时间或把发布机构当招聘单位。
- **验收标准**：
...
当前顺序为：完成 JAI-047 双语迁移基线 → JAI-012 运行与重试能力 → 按原路线实施 JAI-013～JAI-021。用户于 2026-08-29 明确批准一个有边界的 WIP 例外：JAI-021 进入仅自然日验收观测通道时，JAI-022 可从 `develop` 独立并行开发；JAI-021 必须先完成并合并，随后将最新 `develop` 普通合并到 JAI-022，保留两边日志并重新通过完整门禁，绝不通过 rebase 或历史改写规避冲突。JAI-041 的中国移动公开公告最小范围已因国资委阻塞而提前吸收到 JAI-021，不再重复执行；JAI-021 登记的真实完整率差距由 JAI-049 在 MVP 发布闸门前整改；发布闭环稳定后逐站执行其余 JAI-038～JAI-045。JAI-048 作为独立文档 Issue，在任何对应存量文档发生下一次实质修改前完成，不混入功能分支。除本次明确批准的 JAI-021 观测通道外，个人 WIP 上限仍为最多一个主要功能 Issue 加一个小型测试/文档 Issue。动态门户若不满足公开访问和条款边界，记录 `blocked` 并继续下一站，不通过登录、验证码、Playwright 或规避技术措施强行完成数量目标。

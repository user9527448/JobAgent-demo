...
当前顺序为：完成 JAI-037 规划分支 → JAI-012 运行与重试能力 → 按原路线完成 JAI-013～JAI-021，使 MVP 达到 5 个稳定来源 → 发布闭环稳定后逐站执行 JAI-038～JAI-045。个人 WIP 上限为 2：最多一个主要功能 Issue 加一个小型测试/文档 Issue。动态门户若不满足公开访问和条款边界，记录 `blocked` 并继续下一站，不通过登录、验证码、Playwright 或规避技术措施强行完成数量目标。

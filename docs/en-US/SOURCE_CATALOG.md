...
On 2026-08-27, the SASAC public recruitment column failed with retryable `PoolTimeout` for a second consecutive day. The user's normal browser also failed, the public `www` CDN could not be reached, and the official `wap` endpoint exposed an expired certificate, so the W6 stability gate moved it to disabled `blocked` status. China Mobile's official announcement page and same-origin static list/detail JSON returned 200 to bounded GET checks and completed three details in both the 2026-08-27 and 2026-08-29 runs. Because no observation exists for 2026-08-28, the consecutive sequence restarted at day 1 on 2026-08-29. Never infer an application deadline from JSON down-time; retain an image-only body's same-origin image URL without download or OCR; never enter registration, login, resume, or application actions.

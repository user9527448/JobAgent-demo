...
JAI-026 is now started on the independent `feature/jai-026-daily-scheduling-recovery` branch created from the three-way-verified `develop`. Audit the existing collection, parsing, scoring, and report boundaries first, then record the APScheduler, `Asia/Shanghai`, single-instance lock, misfire, stage retry, restart recovery/termination, manual makeup, and end-to-end tracing design in both work logs for project-owner review; implementation starts only after approval. JAI-027 notifications remain out of scope. Never rebase, force push, or rewrite published history. OCR remains deferred to JAI-B01, and JAI-048 remains an independent documentation Issue.

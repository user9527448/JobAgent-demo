...
JAI-026 is now started on the independent `feature/jai-026-daily-scheduling-recovery` branch created from the three-way-verified `develop`. The read-only audit is complete. D-036 uses the stable APScheduler 3 line, one dedicated scheduler process, a persistent PostgreSQL job store, domain run/stage ledgers, and an advisory lock; it executes collection, extraction/validation, matching, and reporting in order with bounded retries, restart recovery, and same-logical-run manual makeup. The project owner approved D-036 and G1–G3 on 2026-09-06, so implementation is proceeding in order; populated business-database migration and live scheduler activation require separate G4 approval. JAI-027 notifications remain out of scope. Never rebase, force push, or rewrite published history. OCR remains deferred to JAI-B01, and JAI-048 remains an independent documentation Issue.

...
| 1 | JAI-027 | G1–G4 complete; M-001/M-002 safely validated; remote feature tip `ff423f1`; unmerged | After `A-001/G5`, complete `0010` plus one named-snapshot live verification and rerun the full gate | Handle G5 only; do not start JAI-028 |
| 2 | JAI-050 | Technical acceptance complete: implementation, design QA, 357-test gate, and container build passed; implementation checkpoint `96fe798` | After JAI-027 integrates first, normally synchronize, reverify, and integrate in order | Do not integrate early or start a later Issue |

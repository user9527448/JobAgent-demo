...
JAI-027 is active on `feature/jai-027-wechat-delivery-idempotency`, whose remote feature tip is `ff423f1ec3b622b6bb934519f57ccf5d08cac885`. Approved D-037 gates G1–G4 are complete: the offline PushPlus adapter, deterministic splitting, durable two-table delivery ledger, fifth pipeline stage, bounded retry/ambiguity protection, operator CLI, paired documentation, and the no-skip 350-test PostgreSQL gate all passed at 85.37% coverage. The business database was last verified at `0009_pipeline_scheduling`; real credentials, migration `0010`, and one named live snapshot send are deferred in the paired manual-action queue until the owner later completes M-001/M-002 and explicitly approves A-001/G5. This is not completion and does not start JAI-028. JAI-050 option 1 implementation, browser design QA, and the 357-test full gate have passed at implementation checkpoint `96fe7984d0a46e5f2c94b2bddebee35adcdf1377`; container-image build verification remains pending as `M-003`, so the Issue remains incomplete and must not merge into `develop` before JAI-027. A temporary spreadsheet-acceleration proposal produced documentation only on an independent audit branch; the owner withdrew it and it remains archived without merge or product scope. The formal critical path is restored to JAI-027 → JAI-050 → JAI-028 → JAI-051 → JAI-029. Never rebase, force push, or rewrite published history. OCR remains deferred to JAI-B01, and JAI-048 remains an independent documentation Issue.
...
| 1 | JAI-027 | G1–G4 complete; remote feature tip `ff423f1`; unmerged | After `M-001`, `M-002`, and `A-001/G5`, complete `0010` plus one named-snapshot live verification and rerun the full gate | Handle only owner prerequisites and G5; do not start JAI-028 |
| 2 | JAI-050 | Implementation checkpoint `96fe798`; implementation, design QA, and 357-test gate passed; later commits are status documentation only | Complete the container-image build after `M-003`; after JAI-027 integrates first, normally synchronize and reverify | Close only container verification and integration preparation |
| Runtime control | `A-006` | Docker restored; one scheduler and one fixed job are active, next at 2026-09-26 08:00; no run rows exist for 2026-09-16 through 2026-09-25 and startup logs contain no explicit misfire event | Owner explicitly chooses “keep” or “stop only scheduler” before the next slot | Infer no missing-date outcome and run no makeup |

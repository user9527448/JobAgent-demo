...
JAI-011, JAI-037, JAI-046, JAI-047, and JAI-012 through JAI-022 have been merged and normally pushed to `develop` in order. The current baseline is JAI-022 non-fast-forward merge `e7948c9225fba32e499786cc8400cf0dd975e4ca`. JAI-021's actual 4/5 China Mobile `PoolTimeout` metric and the user's Day 3 waiver remain intact; JAI-049 continues to track evidence-backed completeness and source risk before the MVP release gate.

JAI-023's hard filters, versioned rule scoring, persisted component explanations, preference-triggered full recomputation, migration, tests, and paired documentation are complete. The latest `develop` is now being normally synchronized into `feature/jai-023-hard-filter-versioned-scoring`; all bilingual WORKLOG histories from JAI-021 through JAI-023 must remain intact and the PostgreSQL-enabled full gate must pass again before merging into `develop`. Apply the same process to JAI-024 afterward. Never rebase, force push, or rewrite published history. OCR remains deferred to JAI-B01, and JAI-048 remains an independent documentation Issue.

...
JAI-021 must produce three consecutive qualified calendar-day observations with the replacement five-source set. The 2026-09-03/04 runs formed qualified days 1 and 2, but China Mobile's public endpoint still hit `PoolTimeout` after three retries on 2026-09-05; only 4/5 source runs succeeded, so the sequence was interrupted. The next qualified calendar-day observation must restart at day 1, without selecting a better result through a same-day rerun. The minimum public-announcement scope originally planned for JAI-041 was reprioritized by the user and absorbed into JAI-021, so no separate JAI-041 branch or duplicate implementation will be created; an internal detail-JSON takedown time must never masquerade as a visible application deadline, and image-only bodies retain only their same-origin image URL without OCR. JAI-049 continues to track the remaining evidence-backed completeness gap before the MVP release gate without changing JAI-022 as the next main-line feature. OCR remains deferred to JAI-B01. Execute JAI-048 as a separate documentation Issue and never mix broad legacy-document migration into feature branches. Integrate remaining official and foreign-enterprise sources one by one under JAI-038–JAI-045 without bypassing login, CAPTCHA, anti-bot controls, or platform terms.

...
JAI-027 completed D-037 G1–G5 and integrated into `develop` through non-fast-forward commit `5c56af3`. The business database is at `0010_notification_delivery`; snapshot 2 was submitted only once, and PushPlus accepted it but its final result could not be confirmed, so the ledger conservatively remains `unknown` and non-resendable. The post-merge PostgreSQL gate passed all 350 tests without skips at 85.53% coverage. JAI-050 integrated and was normally pushed through non-fast-forward commit `dcdd697f6374e6c9d9dee65acf67dfb0aaf281f3`; the post-merge gate passed all 357 tests without skips at 85.80% coverage, and local, tracking, and GitHub refs match. JAI-028 now has a dedicated branch from that baseline. The `/app/` deployment portion of `A-007` is complete, but the scheduler start was blocked before execution and awaits `A-008`, which must explicitly authorize generated report/job content to be sent through PushPlus on up to five automatic runs. After approval, the boundary remains one scheduler, database-backed counting only, and no makeup. A temporary spreadsheet-acceleration proposal produced documentation only on an independent audit branch; the owner withdrew it and it remains archived without merge or product scope. The formal critical path is JAI-028 → JAI-051 → JAI-029. Never rebase, force push, or rewrite published history. OCR remains deferred to JAI-B01, and JAI-048 remains an independent documentation Issue.
...
| 3 | JAI-028 | Started; `/app/` is deployed and scheduler remains stopped | Obtain explicit `A-008` approval for up to five PushPlus content deliveries, then complete five consecutive automatic runs with database evidence | No makeup, prefilled result, or early JAI-051 work |

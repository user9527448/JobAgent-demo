...
JAI-026 has completed approved D-036 gates G1–G4 on the independent `feature/jai-026-daily-scheduling-recovery` branch: stable APScheduler 3 with one dedicated scheduler process, a persistent PostgreSQL job store, domain run/stage ledgers, and an advisory lock; fixed collection, extraction/validation, matching, and reporting order; bounded retries, restart recovery, and same-logical-run manual makeup. The PostgreSQL-enabled full gate passed 313 tests with no skips; the business database migrated to `0009` without drift, and the single scheduler, one live complete makeup, and same-date idempotent reuse all passed. The owner approved safe merge after validation. JAI-027 notifications remain out of scope. Never rebase, force push, or rewrite published history. OCR remains deferred to JAI-B01, and JAI-048 remains an independent documentation Issue.

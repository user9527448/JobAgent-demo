...
| 2 | JAI-050 | Remote stacked-branch tip `96fe798`; implementation, design QA, and 357-test gate passed | Complete the container-image build after `M-003`; after JAI-027 integrates first, normally synchronize and reverify | Close only container verification and integration preparation |
| Runtime control | `A-006` | Docker daemon unavailable on 2026-09-25; no readable ledger evidence for 2026-09-16 through 2026-09-25 | After Docker recovery, first audit Compose, Alembic, jobs, and pipeline ledgers read-only; then the owner decides operating state | Infer no success/failure/misfire and run no makeup |

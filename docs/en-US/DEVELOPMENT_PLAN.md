...
JAI-011, JAI-037, JAI-046, JAI-047, and JAI-012 through JAI-024 have been merged and normally pushed to `develop` in order. The current baseline is JAI-024 non-fast-forward merge `0aa6b233ea8216aecdbe1d1dce4031ad6884a442`. JAI-021's actual 4/5 China Mobile `PoolTimeout` metric and the user's Day 3 waiver remain intact; JAI-049 continues to track evidence-backed completeness and source risk before the MVP release gate.

JAI-025 is complete on its independent `feature/jai-025-top-20-quality-review` branch. The project owner approved the flow-first adjustment and G5 safe merge: 60 explicitly synthetic proposed labels compare `jai-023-v1` with `jai-025-v2`, while a separate bounded run used nine live documents from three sources to complete collection, deterministic extraction/validation, v1 baseline matching, immutable report creation, and idempotent reuse. At least 50 live human labels, automatic post-crawl attachment handoff, Firstjob empty discovery, and China Mobile connectivity are deferred to JAI-049 and must not be represented as completed. After JAI-025 merges and its post-merge gate/three-way verification pass, create JAI-026 from the latest `develop`; JAI-027 notifications remain out of scope. Never rebase, force push, or rewrite published history. OCR remains deferred to JAI-B01, and JAI-048 remains an independent documentation Issue.

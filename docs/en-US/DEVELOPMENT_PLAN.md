...
JAI-011, JAI-037, JAI-046, JAI-047, and JAI-012 through JAI-020 have been merged and normally pushed to `develop` in order. JAI-021 has integrated public NCSS jobs and Shanghai public-institution recruitment notices on its dedicated feature branch. After the public SASAC column remained unreachable from both this machine and the user's browser on two consecutive days, it was marked `blocked` and disabled; with user authorization, the login-free China Mobile recruitment announcements and their public static JSON now provide the fifth active source. All five active sources have offline Adapter contracts, while the three-day stability acceptance remains in progress.

JAI-021 must produce three consecutive qualified calendar-day observations with the replacement five-source set. Although 2026-08-27 was qualified day 1, no real observation exists for 2026-08-28 and the sequence cannot be backfilled or extended. The 2026-08-29 run restarted day 1; 2026-08-30 continued as qualified day 2 with 5/5 source runs, 9/9 details, 0% duplicates, and 80% evidence-backed completeness. A qualified 2026-08-31 run remains. The 2026-08-26 and pre-replacement 2026-08-27 all-source runs remain pre-observations because SASAC hit retryable `PoolTimeout`. The minimum public-announcement scope originally planned for JAI-041 was reprioritized by the user and absorbed into JAI-021, so no separate JAI-041 branch or duplicate implementation will be created; an internal detail-JSON takedown time must never masquerade as a visible application deadline, and image-only bodies retain only their same-origin image URL without OCR. JAI-049 continues to track the remaining evidence-backed completeness gap before the MVP release gate without changing JAI-022 as the next main-line feature. OCR remains deferred to JAI-B01. Execute JAI-048 as a separate documentation Issue and never mix broad legacy-document migration into feature branches. Integrate remaining official and foreign-enterprise sources one by one under JAI-038–JAI-045 without bypassing login, CAPTCHA, anti-bot controls, or platform terms.

On 2026-08-29 the user explicitly approved starting JAI-022 in parallel while JAI-021 waits only for calendar-day observations. JAI-021 keeps its independent branch and acceptance-observation lane and must not close early. JAI-022 must branch independently from verified `develop` without carrying unmerged JAI-021 commits. Merge order is fixed: complete and merge JAI-021 first, normally merge the latest `develop` into JAI-022 while preserving both WORKLOG histories and resolving any paired-document conflict, then rerun the complete gate before JAI-022 may merge. Never rebase or rewrite published history.

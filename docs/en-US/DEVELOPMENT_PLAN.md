...
JAI-011, JAI-037, JAI-046, JAI-047, and JAI-012 through JAI-027 have been merged and normally pushed to `develop` in order. The current `develop` baseline is the JAI-027 non-fast-forward merge `5c56af363066c9bf6d1909e274f83c430b4159b2`. JAI-021's actual 4/5 China Mobile `PoolTimeout` metric and the user's Day 3 waiver remain intact; JAI-025's flow-first exception and deferred quality debt also remain explicitly tracked in JAI-049.

JAI-027 completed D-037 G1–G5 and integrated into `develop` through non-fast-forward commit `5c56af3`. The business database is at `0010_notification_delivery`; snapshot 2 was submitted only once, and PushPlus accepted it but its final result could not be confirmed, so the ledger conservatively remains `unknown` and cannot be resent automatically. The post-merge PostgreSQL gate passed all 350 tests without skips at 85.53% coverage. JAI-050 integrated and was normally pushed through non-fast-forward commit `dcdd697f6374e6c9d9dee65acf67dfb0aaf281f3`; the post-merge gate passed all 357 tests without skips at 85.80% coverage, and local, tracking, and GitHub refs match. JAI-028 now has a dedicated branch from that baseline, and the `/app/` deployment portion of `A-007` is complete. The first scheduled trial on 2026-09-26 produced report snapshot `4`, but the pipeline ended `failed` with `pushplus.access_key_rejected` and its delivery/attempt remain `unknown`, so acceptance is still 0/5. The sole scheduler was stopped under `A-009`. The owner approved A-011 G1 only: implement migration `0011`, append-only development resend audit, and an explicit CLI, then verify them against `_test` with a synthetic provider. The business database remains at `0010`; real resend/makeup, PushPlus access, and scheduler restart are still unapproved. A temporary spreadsheet-acceleration proposal produced documentation only on an independent audit branch; the owner withdrew it and it remains archived without merge or product scope. The formal critical path remains JAI-028 → JAI-051 → JAI-029, with no later Issue started early. Never rebase, force push, or rewrite published history. OCR remains deferred to JAI-B01, and JAI-048 remains an independent documentation Issue.

### 13.1 MVP execution control board (2026-09-25)

| Order | Issue/baseline | Actual state | Closure condition | Allowed next action |
|---:|---|---|---|---|
| 0 | JAI-001–JAI-027 | Integrated and pushed in order | Closed | Immutable baseline only |
...
| 3 | JAI-028 | Paused at 0/5; A-011 G1 migration/audit CLI passed synthetic `_test` verification, with business DB and scheduler unchanged | Clear authentication, separately approve business migration and one real recovery, then complete five consecutive automatic runs | Production still forbids automatic resend; do not start JAI-051 early |

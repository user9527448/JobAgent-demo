...
JAI-011, JAI-037, JAI-046, JAI-047, and JAI-012 through JAI-021 have been merged and normally pushed to `develop` in order. The current baseline is JAI-021 non-fast-forward merge `8cc0b2eb37b5ec7e2c560ce35b687a687da47b43`. JAI-021 daily records cover 2026-09-03 through 2026-09-05; the final day's actual result remained 4/5 because China Mobile hit `PoolTimeout`, and the user explicitly counted it as Day 3 while preserving the metric and endpoint risk. JAI-049 continues to track evidence-backed completeness and source risk before the MVP release gate.

JAI-022's singleton model, read/full-replacement API, schema/enum validation, recomputation signal, unrestricted defaults, migration, tests, and paired documentation are complete. The latest `develop` is now being normally synchronized into `feature/jai-022-single-user-preferences` under the authorized merge train; both JAI-021 and JAI-022 bilingual WORKLOG histories must remain intact and the PostgreSQL-enabled full gate must pass again before merging into `develop`. Apply the same process to JAI-023 and JAI-024 in order. Never rebase, force push, or rewrite published history. OCR remains deferred to JAI-B01, and JAI-048 remains an independent documentation Issue.

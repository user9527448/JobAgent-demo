...
JAI-025 is now active on its independent `feature/jai-025-top-20-quality-review` branch: build at least 50 sanitized, manually reviewable relevance labels, replay `jai-023-v1` unchanged, compare Top 20 false positives and misses under an explicit new version, and document MVP limitations in both languages. JAI-026 scheduling and JAI-027 notifications remain out of scope. Never rebase, force push, or rewrite published history. OCR remains deferred to JAI-B01, and JAI-048 remains an independent documentation Issue.

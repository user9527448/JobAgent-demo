...
JAI-021 now has consecutive daily records from 2026-09-03 through 2026-09-05. The first two days qualified normally; on day 3, China Mobile's public endpoint still hit `PoolTimeout` after three retries and the actual source success rate was 4/5. The user explicitly judged that isolated failure likely external to the crawler and authorized conditionally counting the day while preserving the actual metrics and endpoint risk, then starting the safe merge train. The minimum public-announcement scope originally planned for JAI-041 was reprioritized by the user and absorbed into JAI-021, so no separate JAI-041 branch or duplicate implementation will be created; an internal detail-JSON takedown time must never masquerade as a visible application deadline, and image-only bodies retain only their same-origin image URL without OCR. JAI-049 continues to track the remaining evidence-backed completeness gap before the MVP release gate. OCR remains deferred to JAI-B01, and JAI-048 remains an independent documentation Issue.

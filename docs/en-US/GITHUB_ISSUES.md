...
- **Status**: deterministic grouping, rendering, snapshot migration, API, tests, and paired documentation completed on the dedicated branch on 2026-09-03. Seven focused PostgreSQL tests passed, and the complete gate passed all 252 tests with no skips and 88.53% coverage. Feature commit `ffa065f` was normally pushed and verified across all three refs.
...
- **Status**: D-037 G1–G4 implementation, paired documentation, and the 350-test PostgreSQL full gate passed; the remote feature tip is `ff423f1ec3b622b6bb934519f57ccf5d08cac885`. M-001/M-002 are complete and safely validated; business migration `0010` and one named-snapshot live verification still require `A-001/G5`, so the Issue is incomplete and not integrated into `develop`.
...
- **Status**: option 1 implementation, browser design QA, the 357-test Python full gate, and M-003 container-image build verification all passed at implementation checkpoint `96fe7984d0a46e5f2c94b2bddebee35adcdf1377`. Technical acceptance is complete; after JAI-027 integrates first, synchronize, reverify, and integrate this branch into `develop` in order.
...
JAI-021 through JAI-026 have completed and merged into `develop` in order; the current baseline is the JAI-026 non-fast-forward merge `a9e9b643b629e5632015778549917f44bd658586`. JAI-027 has completed approved D-037 G1–G4 implementation, paired documentation, and the no-skip 350-test PostgreSQL gate at 85.37% coverage on its remote independent feature tip `ff423f1ec3b622b6bb934519f57ccf5d08cac885`. M-001/M-002 are complete and safely validated; business migration and one named live snapshot test remain behind A-001/G5, so do not mark the Issue complete. JAI-050 implementation checkpoint `96fe7984d0a46e5f2c94b2bddebee35adcdf1377` contains the passed option 1 implementation, browser design QA, 357-test full gate, and M-003 container-build verification. Technical acceptance is complete, but it must not merge into `develop` before JAI-027. The temporary spreadsheet-acceleration proposal was withdrawn and remains only as an unmerged audit record; it is not part of this backlog. The formal sequence remains JAI-027 → JAI-050 → JAI-028 → JAI-051 → JAI-029. Read-only evidence after Docker recovery on 2026-09-25 shows no run rows for 2026-09-16 through 2026-09-25 and no explicit misfire event in startup logs. The owner then approved stopping only the scheduler; `db`/`api` remain healthy, while the fixed job and existing ledgers remain intact. Infer no missing-date outcome and run no makeup. JAI-051 migration still requires U3. Never rebase, force push, or rewrite history. JAI-041 was absorbed into JAI-021; JAI-049 tracks live completeness and source risk before the MVP release gate. Execute the remaining JAI-038–JAI-045 sources one at a time after the release loop is stable, and keep JAI-048 independent. If a dynamic portal cannot satisfy public-access or terms boundaries, record `blocked`; never force integration with login, CAPTCHA, Playwright, or evasion.

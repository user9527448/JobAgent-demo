...
| 2026-09-05 | Conditionally accepted day 3 by explicit user waiver | 80% (4/5) | 100% (6/6) | 0% | 86.67% (26/30) | China Mobile's public announcement endpoint exhausted three retries with a retryable `PoolTimeout`; the user accepted this day as an external endpoint/path anomaly while preserving the actual 4/5 result and requiring follow-up monitoring |

The pre-replacement comparable diagnostic composite was 82.5% across NCSS (12/15), Jiangsu (6/10), and Shanghai (15/15). It was not a single all-source run and was not counted as day 1. The 2026-08-27 replacement run was interrupted by the missing 2026-08-28 observation; the later 2026-08-29/30 pair was interrupted by the missing 2026-08-31 through 2026-09-02 observations. The 2026-09-03/04 runs qualified normally. On 2026-09-05 the user explicitly waived the all-source-success condition for that day, judged the isolated China Mobile `PoolTimeout` likely external to the crawler, and authorized counting the recorded run as day 3. This closes the three-day acceptance by an explicit product-owner exception, not by rewriting the run as 5/5; post-merge monitoring must retain the endpoint risk. Completeness remains evidence-only: missing fields are never invented to raise the percentage.

## 6. Known gap and corrective Issue

JAI-049 tracks the remaining evidence-backed completeness gap before the MVP release gate. It must improve or replace low-value broad announcement inputs without inventing organization/deadline values. Three consecutive daily records now exist for 2026-09-03 through 2026-09-05; the final day is accepted only under the explicit waiver recorded above.

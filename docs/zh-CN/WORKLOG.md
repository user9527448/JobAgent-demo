...
| JAI-027 | D-037/G1～G4 已批准并完成；G5 待审批 | 远程 feature 末端 `ff423f1` | PostgreSQL 完整门禁以 350 项测试和 85.37% 覆盖率通过；业务迁移、凭据和真实投递仍未授权 |
| JAI-050 | 实现/design QA/完整门禁完成；`M-003` 待处理 | 实现检查点 `96fe798` | 357 项测试和 85.65% 覆盖率通过；后续提交只同步状态文档；容器构建验证与按序集成仍未关闭 |
...
1. 负责人通过预拉取 `node:24-alpine` 完成 `M-003`；随后由 Agent 在不重建当前服务的前提下重跑 JAI-050 容器构建，关闭其最后一项技术验收。
2. 负责人准备好后完成 `M-001`、`M-002`，再单独批准 `A-001/G5`。只有 JAI-027 G5 关闭后才能先集成 JAI-027；随后普通同步、复验并集成 JAI-050。

...
1. 提交已完成的 JAI-027 G4 变更与完整门禁证据供审核。把 `0010` 应用于已有数据的业务库、注入凭据、重启 scheduler 或对一份明确快照执行一次真实 PushPlus 测试前，必须另行取得 G5 批准并完成影响复核。
2. 修改双语开发计划/Backlog 或增加前端依赖前，必须取得 D-038/U1-R 审批；如获批，在 JAI-027 集成后通过专用 JAI-050 分支把 JAI-050/JAI-051 与调整后的 JAI-031 范围写入四份计划文件，再准备三个 U2 视觉方向与成对 DESIGN.md 草案。

...
| JAI-028 | 已启动；`/app/` 已部署，scheduler 启动等待 `A-008` | `feature/jai-028-e2e-unattended-trials` | 需明确批准最多五次自动 PushPlus 内容外发；不补跑 |
...
1. 按 `A-007` 只使用已验证镜像重建 `api`，期间保持 scheduler 停止；核验健康状态、`/app/` 和容器
   身份。
2. 取得 `A-008` 明确批准最多五次自动运行生成的日报/岗位内容通过 PushPlus 外发；之后才能启动恰好
   一个 scheduler 并核验下一时刻。

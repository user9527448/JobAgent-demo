...
| JAI-028 | 当前暂停且为 0/5；AccessKey 复测仍返回 provider `403`，scheduler 已停止 | `feature/jai-028-e2e-unattended-trials` | D-041 已原则批准受控开发恢复，但任何补跑/重发前仍需 A-011 的实现与数据审批 |
...
1. 保持 scheduler 停止并继续修正 PushPlus 安全 IP，直到另行批准的仅鉴权检查成功取得 AccessKey；
   最近一次真实检查仍为 `403`。
2. 在实现迁移 `0011` 和显式、仅开发环境、只追加的重发授权路径前取得 A-011 批准；不得改写原有
   `unknown` 尝试。
3. 阻塞和审计路径均验证后，再分别批准真实重发/补跑及新的无人值守观测窗口；JAI-028 关闭前不得

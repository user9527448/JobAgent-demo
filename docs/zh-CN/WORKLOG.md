...
| JAI-027 | D-037/G1～G4 已批准；G4 文档完成，完整门禁受 Docker 阻塞 | `feature/jai-027-wechat-delivery-idempotency` | 投递文档/配置已同步；业务迁移、凭据和真实投递仍未授权 |
...
### 2026-09-10 — JAI-027 G4 文档完成；完整门禁等待 Docker
...
1. 由项目负责人手动启动 Docker，然后只读复核业务台账，并对 `jobagent_test` 运行完整 PostgreSQL `scripts/check.py` G4 门禁。
2. 把 `0010` 应用于已有数据的业务库、注入凭据、重启 scheduler 或对一个明确快照执行一次真实 PushPlus 测试前，必须另行取得 G5 批准并完成影响复核。
3. 修改双语开发计划/Backlog 或创建 JAI-050 前，必须取得 D-038/U1 审批；如获批，在 JAI-027 集成后通过专用 UI 分支把 JAI-050/JAI-051 与调整后的 JAI-031 范围写入四份计划文件。
4. 2026-09-07 之后只按台账证据报告；未经日期级批准不得推断失败或补跑。JAI-028 无人值守验收与 JAI-029 发布仍保持独立。

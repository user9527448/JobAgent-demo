...
> 最后更新：2026-09-26
>
> 当前分支：`feature/jai-028-e2e-unattended-trials`
...
### D-034 已批准的历史评审集缺失恢复路径

项目负责人已于 2026-09-05 批准本恢复路径与 G1。方案保留 JAI-025 原定的历史/人工评审目标，不把合成试运行集直接声明为充分验收。以本地 `jobagent` 数据库作为受控证据库：从 Alembic `0003_attachment_storage` 升级到仓库 head；在事务中只初始化网站库里已经标记为 `active` 且 `enabled` 的五个来源；在任何持久化线上运行前，为现有手工采集路径增加向后兼容的可选详情条数上限。采集仅访问公开数据，并发为 1、请求间隔至少一秒、不得绕过登录/验证码/访问控制；详情尝试总数不超过 60，至少三个来源提供样本，单一来源进入评审的候选不超过 30 条。

通过既有服务持久化不可变原文、确定性抽取和证据。包含原始来源信息的评审表及 URL 映射只保存在被忽略的 `data/` 下；项目负责人对至少 50 个不同岗位完成相关性类别/依据确认后，才提交脱敏基准。不得删除未入选来源记录，只能通过评审清单排除。已经发布的 feature 历史不改写：如果真实标注要求改变暂定 `jai-025-v2` 规则，最终候选必须使用新评分版本，不能静默修改 v2。JAI-026 调度、JAI-027 投递、JAI-030 来源维护 API、LLM 调用和新增来源 Adapter 均不属于本方案。

审批闸门不可跳过：G1 审批数据库升级、活动来源初始化与有界采集代码改动；G2 在持久化 HTTP 详情请求前审批只读发现数量及分来源配额；G3 在调权前确认人工标注与脱敏转换；G4 在查看调权前后 Top 20 误推荐/漏召回后审批最终规则/权重与版本号；G5 在双语文档及 PostgreSQL 完整门禁通过后审批完成和安全合并。遇到不一致、访问限制、不同岗位不足或质量权衡时，必须停在当前闸门并记录；不得自动扩大范围或执行破坏性回滚。

### D-035 先闭合可执行 MVP 流程，再优化质量与样本量

项目负责人于 2026-09-05 把当前优先级从“等待 50 条线上评审岗位才能推进”调整为“先完成并验证现有端到端流程”。来源 ID 1～5 的 G2 配额 `3/0/2/5/0` 已获批准。运行仍严格限制为 3 个可达来源、最多 10 条公开详情；既有并发、限速、证据和访问控制边界不变。
...
### D-040 撤回临时表格提案并恢复正式路径

2026-09-23，项目负责人撤回了表格数据源加速方案。其独立审计分支以未合并文档记录形式保留在远程
`53ada588`，但 JAI-052/JAI-053 不进入当前 Backlog 或完成度统计。项目不采用由该表格派生的产品实现、
业务数据导入、数据库变更、scheduler 变更或来源契约。工作恢复到已经批准的
JAI-027 → JAI-050 → JAI-028 → JAI-051 → JAI-029 关键路径；本提案后续不再访问该共享资源。

### D-041 开发恢复必须显式且只追加

2026-09-26，负责人调整开发测试规则：允许受控补跑与重发，但每次都必须记录操作者授权、原因、原有
不明确身份、新 provider 尝试和最终结果。这不授权静默重试 `unknown`，也不削弱生产环境幂等。
拟议实现保持生产默认禁止，仅提供显式的开发环境命令和重复风险确认，在任何 provider 调用前先写入
持久只追加的操作记录，并保留全部既有尝试与流水线阶段行。业务迁移及第一次真实重发仍受 A-011
审批。除非负责人另行改变 JAI-028 的五次验收定义，人工恢复的开发运行与干净无人值守成功分开统计。

### D-042 A-011 G1 分离流水线补跑证据与 provider 重发授权

负责人批准 G1 实现及仅 `_test` 验证，但未批准业务迁移或真实操作。既有补跑保持显式命令，其不可变
逻辑运行、`trigger=makeup` 和编号阶段行已经构成只追加证据。因此迁移
`0011_delivery_operator_audit` 补齐 provider 边界证据：同一 UUID 下记录不可变的 `authorized`、
`started`、`completed` 事件，在唯一一次 provider 提交前持久化授权，并由数据库触发器拒绝更新/
删除。新 `resend` 命令仅在 `development` 可用，要求有界原因与重复风险确认，不修改原尝试，也不
隐式重试提交。
...
### 2026-09-05 — JAI-025 缺失数据恢复方案已登记待审批

- 再次只读核验本地状态：业务数据库处于 Alembic `0003_attachment_storage`；`sources`、`raw_documents`、`job_posts`、`job_positions` 均为 0 行；仓库 head 为 `0008_daily_report_snapshots`。
- 现有 `scripts/manage_crawl.py run` 要求数据库存在与网站库精确匹配的启用来源，并会持久化一次来源发现返回的全部条目；当前没有操作员可指定的条数上限。网站库中已有五个 `active`/`enabled` 公开来源，但运行时来源表为空，因此目前不能启动持久化采集。
- 已登记拟议决策 D-034 及 G1～G5 闸门。推荐路径保留原计划：升级空的本地业务 Schema，仅以事务方式初始化既有获准活动来源，新增带测试的可选详情上限，跨至少三个来源最多采集 60 条公开详情，运行既有确定性抽取/证据持久化，把面向来源的评审材料留在本地忽略目录，并且只提交经负责人确认的脱敏基准。
- 备选 B 是接受当前合成集作为范围替代；速度更快，但不满足原定历史样本目标，因此不推荐。备选 C 是等待 JAI-026 先积累数据；这会颠倒 JAI-025 → JAI-026 的计划顺序，未经明确调整优先级也不推荐。
- 等待审批期间没有执行迁移、数据库插入、持久化采集、其他代码改动、评分重调、完成勾选、合并或破坏性操作。
- 下一步：由项目负责人审核并批准/否决 G1 与完整 D-034 路径。只有取得明确批准后才开始实施；后续闸门仍需分别提供证据并再次审批。

### 2026-09-05 — D-034 与 G1 已批准

- 项目负责人已明确要求按登记的设计执行。本次只批准 D-034 与 G1：为手工采集增加向后兼容的有界选项及测试/文档；把空的本地业务数据库从 `0003_attachment_storage` 升级到仓库 head；并在事务中只初始化网站库里已经标记为 `active` 且 `enabled` 的五个来源。
- G1 不授权持久化线上采集、标注确认、评分重调、完成状态或合并。在 G1 证据登记完毕且负责人另行审批 G2 的发现数量和配额之前，不得持久化任何来源详情请求。
- 执行顺序：先实现并验证可选条数上限；记录迁移前版本/计数；执行现有已测试的 Alembic 升级且不做破坏性降级；核验 head/Schema 漂移；在单个事务内插入精确匹配既有网站库身份的来源并执行写后相等检查；最后运行相称/完整门禁并记录最终状态。

### 2026-09-05 — JAI-025 G1 完成并备妥 G2 证据

- 新增可选的 `run --limit N`。它按来源稳定顺序选取发现结果前 `N` 条，同时记录入选数和发现总数，拒绝非正数上限，且不得与失败 URL 重试筛选组合。原有不限额运行和重试行为保持不变。配对采集文档与回归测试已同步更新。
- 升级前业务数据库位于 `0003_attachment_storage`，来源、原始公告、公告和岗位均为 0 行。升级后首次复核因沙箱无法访问 Docker 命名管道而没有取得密码，Alembic 认证失败且未写数据库。随后在获授权的宿主环境中，以从运行容器取得的进程级 URL 重试；没有打印、保存或提交凭据。
- Alembic 当前为 `0008_daily_report_snapshots (head)`，`alembic check` 未发现待升级操作。受保护的单事务先要求 `sources` 为空，再精确插入现有 5 个活动/启用网站库身份，校验 Adapter 集合与数量后提交。来源 ID 1～5 依次为 NCSS、Firstjob、江苏人事考试、上海事业单位和中国移动，均已启用并使用网站库间隔；`crawl_runs`、`raw_documents`、`job_posts`、`job_positions` 仍全为 0。
- 最终启用 PostgreSQL 的完整门禁已通过 Ruff format/lint、139 个源文件的 Mypy、294 项无跳过测试和 87.80% 覆盖率，包含“不得对失败重试应用限额”的直接不变量测试。
- 只读列表发现仅用于准备下一审批输入。沙箱内尝试在其网络边界失败；宿主环境重试得到 NCSS 3、Firstjob 0、江苏 2、上海事业单位 5 条，中国移动耗尽连接重试。全程未请求详情，也未写数据库或文件。因此拟议 G2 配额为 3/0/2/5/0（3 个来源共 10 条详情）。这些公告的附件可能展开为至少 50 个岗位，但无法保证；如果最终少于 50 个不同岗位，流程必须停下等待新决策，不得自动扩源。
- G1 已完成。在项目负责人审核并明确批准或修改 G2 前，仍不得执行持久化详情请求、抽取、标注确认、评分重调、完成勾选或合并。
- 已使用仓库本地作者 `user9527448 <2537759248@qq.com>` 创建 G1 实施提交 `4b737e4e8eeab3bda2e45af9c3adf0cd6d183c4c`，并通过此前验证的命令级临时代理普通推送，未修改 `origin` 或持久 Git 配置。本次仅状态更新前，本地 HEAD、跟踪引用与 GitHub `ls-remote` 均与该提交一致，工作区干净。

### 2026-09-05 — D-035 优先级调整与 G2 已批准

- 项目负责人要求优先闭合可执行流程，并把新发现的优化需求记录后再调整。这一指示批准拟议 G2 配额 `3/0/2/5/0`，同时保留详情总数 10、覆盖 3 个来源、并发 1、至少一秒限速、仅公开访问且不得绕过的边界。
- 先前“线上运行不足 50 个不同岗位即立刻停止”的要求仅对本轮流程验证被取代。任何差额仍是显式质量债务；合成标注继续明确标记为合成，JAI-025 不得声称完成并不存在的 50 条历史人工评审。
- 执行边界：只持久化获批且可达的详情，检查运行/数据库证据，在既有确定性解析、抽取、匹配、报告组件支持的范围内继续，并记录每个不支持的交接或缺失操作命令。未经后续决策，不扩大配额、不新增来源/Adapter、不调权，也不实现 JAI-026/JAI-027 行为。

### 2026-09-05 — JAI-025 有界真实流程验证完成

- 只运行获批的来源 ID 与上限。采集运行 1（NCSS，上限 3）发现并创建 2 条文档；运行 2（江苏，上限 2）创建 2 条；运行 3（上海事业单位，上限 5）创建 5 条。三次运行全部成功，9 条尝试详情全部持久化且无失败。Firstjob 与中国移动没有执行持久化运行，未扩大获批总量。
- 9 条当前文档均包含可见正文。附件表仍为空，因为当前手工采集只保存详情正文，不会把发现的 PDF/XLSX 链接交给 `AttachmentStorageService`。尽管如此，确定性重解析版本 `jai-025-live-v1` 对全部文档完成：9 条公告、2 个岗位、38 条字段证据、41 条校验问题。1 个岗位可推荐、1 个被阻断；7 份以附件为岗位表的公告未从正文生成岗位。缺失值保持缺失。
- 保持默认空偏好（即不限制）的值不变并触发一次重算；在 G4 前刻意使用已发布基线 `jai-023-v1`，没有把候选 v2 当成获批线上规则。匹配处理 2 个岗位，1 个通过、1 个过滤，创建 2 条结果。日报版本 `jai-024-v1` 为 2026-09-05 创建一个快照，分组数量为优先投递 1、即将截止 0、今日新增 1、需要确认 2。
- 第一次直接调用一次性服务时使用了 Windows 默认 Proactor loop；异步 psycopg 要求 selector loop，因此在读取偏好前失败，未写偏好、匹配或日报。重试采用仓库命令相同的 `SelectorEventLoop` 模式后成功。再次执行时匹配返回 `not_required`、处理 0 行，日报以相同内容哈希复用快照 1。
- 新增一项合成 PostgreSQL 端到端回归，在一个测试中保护原文重解析、校验、默认偏好不变更新、v1 匹配、日报四组、匹配空操作和不可变日报复用。定向 Ruff 与 PostgreSQL 测试通过。直接对单个测试文件调用 Mypy 时，项目导入被解析为缺少类型标记的已安装模块并报告 import 错误；最终门禁将以仓库配置目标运行 Mypy，作为权威结果。
- 已把延期质量工作登记到 JAI-049：自动有界附件衔接、Firstjob/中国移动来源诊断、至少 50 个不同真实岗位的人工标注，以及该基准要求改规则时使用新的评分版本。60 条合成结果和 2 岗位真实冒烟继续分别标识，不构成生产质量声明。
...
### 2026-09-06 — JAI-026 G1～G3 实施与测试验收完成

- G1 增加稳定 APScheduler 3 依赖及有类型的计划/重试配置；迁移 `0009_pipeline_scheduling` 与对应 ORM 模型统一管理 `apscheduler_jobs`、唯一身份的 `pipeline_runs` 和追加式编号 `pipeline_stage_runs`。PostgreSQL 仓储提供逻辑运行 upsert/查询/恢复与阶段尝试状态转换，session advisory lock 覆盖完整流水线。
- 首轮 G1 PostgreSQL 定向运行有两项迁移漂移失败，原因是由调度器管理的 APScheduler 表未登记到 ORM metadata；当时台账和 advisory lock 行为已经通过。增加最小 `APSchedulerJob` 映射后，在不改变调度器表结构的前提下对齐 Alembic 自动生成检查；重跑的四项迁移/仓储/锁检查全部通过。
- G2 实现可注入协调器，固定按采集 → 抽取/校验 → 匹配 → 日报执行，包含有界临时重试、部分来源成功后继续、永久/意外/取消终止、重启中断与恢复、终态复用。强制匹配使用既有 `CURRENT_SCORE_VERSION` 和固定计划时刻，不改变偏好值。生产阶段输出保留采集运行、输入文档、公告/岗位、匹配结果、日报快照 ID 及准确版本。
- G3 增加可导入的固定调度目标、PostgreSQL 持久 job store、本地日期补跑与运行查询 CLI、Windows selector loop 处理，以及唯一一个 Compose scheduler 服务；同步更新 `.env.example`、镜像配置、中英文调度/数据库指南及两份文档索引。没有增加投递、附件衔接、新来源/Adapter、LLM 或 JAI-027 行为。
- 受保护的 PostgreSQL 验收只使用 `jobagent_test` 与合成公网采集输入。六项定向迁移、锁、匹配、完整流水线、幂等复用和遗留阶段恢复检查通过。端到端用例真实执行确定性抽取、匹配、日报和持久化：两个逻辑日期产生 2 条流水线运行、含 1 次中断尝试的 9 条阶段记录、2 条采集运行、1 条幂等公告、2 条版本化匹配和 2 份日报快照。
- 启用 PostgreSQL 的权威 `scripts/check.py` 门禁通过：Ruff format 检查 232 个文件，Ruff lint 通过，Mypy 检查 155 个源文件通过，313 项测试全部通过且无跳过，覆盖率 86.20%。本次文档更新前 `git diff --check` 也已通过。
- 双语标题数在计划 45/45、Backlog 71/71、WORKLOG 69/69、数据库指南 6/6、调度指南 5/5 上一致；两份 Backlog 保持相同的 178 个 Issue 引用顺序。首次临时相对链接命令没有为仓库根文件的空父路径补 `.`，因此检查器自身失败；修正后的只读命令检查 67 份已跟踪/新增 Markdown，未发现失效相对链接。文档更新后的最终 Ruff format/lint、Mypy 与 `git diff --check` 仍保持通过。
...
### 2026-09-14 — JAI-050 正式前端基底启动

- 已在计划提交 `ff423f1ec3b622b6bb934519f57ccf5d08cac885` 上按获批堆叠策略创建 `feature/jai-050-production-ui-foundation`；分支继承 JAI-027 G1～G4，但不得先于 JAI-027 合入 `develop`。
- 已重新核验方案 1 原始参考并将其作为文档资产纳入 Git；原图为 `1484x1060`，SHA-256 为 `09D55CFD2B5550E6D4D199004F690992DA90703B73AC38828808F94295AF8DE4`。参考只约束构图和层级，生成图中的业务数据不得进入实现。
- 已新增成对 DESIGN.md，固定信息架构、视觉 token、响应式/无障碍规则、只读 API 清单、`0009` 迁移前投递状态兼容、FastAPI 同源发布和 design QA 闸门；两份文档索引及中英文 AGENTS UI 必读项同步更新。
- 本阶段尚未安装前端依赖或修改应用代码；下一步先验证并提交设计基线，再审计后端查询与静态发布边界后实施。
...
### 2026-09-15 — JAI-050 实现已提交

- 已使用核验过的仓库本地作者 `user9527448 <2537759248@qq.com>` 创建范围内 feature 提交 `1e66afa`（`feat: add production morning briefing foundation`）；其中包含已审查的应用、测试、双语文档、验收状态更新和 08:00 后的事实台账记录。
- 分支仍堆叠于 JAI-027，尚未推送或合并。`M-003` 完成容器镜像构建验证前，JAI-050 保持未完成；不会从该提交启动 JAI-028/JAI-051/JAI-029。

### 2026-09-15 — MVP 计划与执行对账

- 负责人反馈整体进度已缺少与原计划一致的直观感受。只读审计确认 `develop` 仍停在已集成的 JAI-026 基线，JAI-027 是下一项未完成 Issue，JAI-050 是负责人明确批准的堆叠例外，JAI-028/JAI-051/JAI-029 均未启动。分支图保持线性：JAI-027 末端 `ff423f1` 是 JAI-050 实现检查点 `facbdac` 的祖先，后者比 `develop` 超前 13 个提交；没有改写历史。`git ls-remote --heads` 未返回 JAI-027 或 JAI-050 远程引用，确认两者仍只存在于本地。
- 清晰度下降源于关闭闸门分散在 Backlog、人工队列和 WORKLOG，而不是发生了未登记的 Issue 改序。现已在两份开发计划加入成对 MVP 执行控制面板，并扩展 JAI-027 验收项，明确区分已完成的 G1～G4 证据和待执行 G5；Issue 顺序与范围不变。
- 强制关键路径为：关闭 JAI-027 → 完成 JAI-050 容器验证并按序集成 → JAI-028 五次无人值守运行 → JAI-051 反馈 → JAI-029 发布。前序未关闭时不创建后续分支；唯一例外必须由负责人批准、双语留档且保持集成顺序，当前只有 JAI-050 属于该例外。

### 2026-09-25 — 表格提案撤回后恢复原计划

- 撤回提案以未改写历史的方式保留在远程审计分支 `53ada5883cc17fe1d2bf3718aa4dceb8e29d6bf7`；它没有合入 JAI-050 或 `develop`，也没有引入产品实现、来源数据复制、业务数据库写入、scheduler 变更或新的活动 Issue。
- 已回到 `feature/jai-050-production-ui-foundation`。本地 HEAD 与缓存的远程分支引用均为 `96fe7984d0a46e5f2c94b2bddebee35adcdf1377`；工作区干净，仓库本地作者仍为 `user9527448 <2537759248@qq.com>`。
- 两份计划、Backlog、人工操作队列和 WORKLOG 已恢复单一正式路径 JAI-027 → JAI-050 → JAI-028 → JAI-051 → JAI-029。历史条目不改写，只更新当前状态并追加事实。
- 2026-09-25 的 A-006 只读审计发现 Docker 客户端为 29.6.2，但守护进程管道不存在，因此无法读取 Compose、Alembic、APScheduler 和流水线台账。2026-09-16 至 2026-09-25 不推断成功、失败或 misfire；没有启动 Docker、补跑、迁移、投递、来源请求或业务数据库写入。
- 负责人手动启动 Docker 后，提升权限的只读核验发现 Docker Desktop 4.85.0/Engine 29.6.2、一个健康 `db`、一个健康 `api` 和一个 scheduler。业务数据库仍为 `0009_pipeline_scheduling`；通知投递表不存在，也没有发生 `0010` 迁移。恰有一个 `jobagent.daily-pipeline.v1` 作业，下次指向 `Asia/Shanghai` 2026-09-26 08:00。
- 流水线证据仍为两条成功运行：2026-09-06 补跑和 2026-09-15 计划运行，每条都恰有四个首次尝试成功阶段。2026-09-16 至 2026-09-25 没有运行行。启动日志只显示作业登记与 scheduler 启动，没有显式 misfire 事件，因此不把十个缺失日期分类为成功、失败或 misfire。业务计数仍为 13/43/52/11/15/3，与末次记录一致。
- 本地仍无 `node:24-alpine`，所以 `M-003` 继续待处理。本次审计没有拉取镜像、构建/重建容器、变更 scheduler、补跑、迁移、投递、访问来源或写入业务数据库。
- 负责人随后明确批准“只停止 scheduler”。`docker compose stop scheduler` 只停止该服务，退出码为 143；一个健康 `db` 和一个健康 `api` 继续运行。停止后的只读 SQL 确认 `0009_pipeline_scheduling`、保留的固定作业行及其存储的 2026-09-26 08:00 时刻、两条成功流水线运行和八条成功阶段记录。没有补跑、迁移、投递、来源请求或业务写入。此后任何 scheduler 重启都需要新的明确批准。
- 负责人完成了 M-001/M-002/M-003。安全核验只读取密钥是否存在：两项 PushPlus 变量各恰好出现一次且非空，`.env` 被 Git 忽略，没有输出或在其他位置持久化任何值，也没有请求 provider。
...
### 2026-09-25 — JAI-050 已集成并启动 JAI-028

- JAI-050 feature 末端 `8c856b1af39b8d9ac9f199110cec925905faa8a8` 已普通推送，并通过
  非快进 `develop` 提交 `dcdd697f6374e6c9d9dee65acf67dfb0aaf281f3` 合入。合并后前端门禁和
  PostgreSQL 完整门禁再次通过：357 项测试、无跳过、覆盖率 85.80%。本地 `develop`、
  `origin/develop` 与 GitHub 一致，JAI-027/JAI-050 feature 末端均为其祖先。
- 版本控制审计发现一项有边界的历史分叉：GitHub `main` 在本地 `main` 基线上还有 2026-08-27 的
  README-only 提交 `8a9a3235956e3b8811052e8cf025cad088e98619`。它不含产品代码，JAI-028 不合并
  或改写；JAI-029 发布前必须显式对账。当前活动 `develop` 线和范围内 feature 引用仍干净、已发布且
  可审计。
- 已从 `dcdd697f...` 创建 `feature/jai-028-e2e-unattended-trials`。负责人批准 `A-007`：只使用
  已验证的 JAI-050 镜像重建 `api`，核验 `/app/` 后启动恰好一个 scheduler，执行连续五次自动运行。
  允许的影响为公开来源请求、相应业务写入及流水线内置幂等投递；补跑、人工重发、扩容、JAI-051 和
  JAI-029 均不在范围内。
- 试运行结果不得预填。只有只读数据库证据确认计划身份、流水线/阶段终态、不可变日报、投递台账、
  指标及无重复公告/通知后才计数。任何失败、重复、非终态、投递歧义或多个 scheduler 实例都会暂停
  验收并先行诊断。
- 已只使用镜像 `sha256:1662d4ec25639fa7657ee34ca95b906389df51535c28e29bb90118af76c48292`
  重建 `api`。新容器健康，`/health/live`、`/health/ready`、`/app/` 和 `/dashboard/briefing` 均返回
  HTTP 200；部署期间 scheduler 一直停止。
- 启动前快照确认 Alembic `0010`、一个固定作业指向 `Asia/Shanghai` 2026-09-26 08:00、两条成功
  流水线、八条成功阶段、三份日报，以及此前一条 `unknown` 投递/尝试。scheduler 启动操作在执行前
  被拒绝，因为现有批准没有明确授权最多五次自动运行生成的日报/岗位 payload 通过 PushPlus 外发。
  拒绝后的证据未变化，scheduler 仍为 `Exited (143)`；新增 `A-008` 记录这一更窄授权要求。
- 负责人已明确批准 `A-008`：最多五次自动计划运行可以通过已配置 PushPlus 目的地外发生成的岗位/
  日报内容，并可访问已批准公开来源、写入业务数据。仍禁止补跑、人工发送/重发、额外通知和 scheduler
  扩容；任何失败、重复、非终态或投递结果不明确都必须暂停并报告。
- 首次重建 scheduler 选中了缓存镜像 `sha256:80744aaf...`。只读哈希核验发现其流水线源码为当前
  版本，但通知服务早于 JAI-027 保守 `unknown` 修复；已在首个 08:00 时刻前且台账变化前停止。
  随后重建 `jobagent-scheduler:latest`，无网络镜像核验确认两份关键源码哈希均与本地一致。当前恰好
  一个 scheduler 使用 `sha256:7138bffe...` 运行，重启次数为零，下一时刻为 2026-09-26 08:00；
  启动没有产生补跑、流水线行或通知尝试。
- 创建五日核验 heartbeat 的操作在创建前被拒绝：周期读取台账、异常时停止 scheduler、修改成对
  文档及 Git 提交/推送需要单独明确授权。没有创建自动化，scheduler 状态也未变化；新增 `A-009`
  记录缺少的权限。
- 负责人已明确批准 2026-09-26 至 2026-09-30 的 `A-009`，包括自动读取 Docker/业务台账、异常时
  停止 scheduler 并通知、成功时执行成对文档检查及 feature 分支普通提交/推送，以及第五次成功后
  停止 scheduler 并执行 JAI-028 最终门禁。仍禁止补跑、重发、合并 `develop` 和后续 Issue 工作。
- 已创建活动线程自动化 `jai-028`，按 `Asia/Shanghai` 每天 08:15 核验，共五次；因为创建时已晚于
  9 月 25 日核验时刻，实际覆盖 9 月 26–30 日。读取保存状态确认其为活动。首次本地调用存在引号
  语法错误，第二次因立即创建不能包含 DTSTART 被拒绝；两次均未创建自动化或改变运行态。最终接受的
  周期省略 DTSTART，同时保持恰好五个未来时刻。
- 负责人约在 `Asia/Shanghai` 23:40 重启 Docker Desktop 后，只读审计确认 `db`/`api` 健康、恰好
  一个 scheduler 仍使用同一已核验 `sha256:7138bffe...` 镜像，重启次数为零。固定作业仍指向
  2026-09-26 08:00；台账仍为两条成功运行、八条成功阶段、三份日报及此前一条 `unknown` 投递/
  尝试；`/app/` 与两个健康接口均返回 HTTP 200。重启没有产生补跑、提前流水线行或通知尝试。

### 2026-09-26 — JAI-028 第一次计划试运行失败并暂停验收

- 固定作业在 `Asia/Shanghai` 08:00 恰好触发一次。权威数据库记录为流水线运行 `4`：
  `trigger=scheduled`、`status=failed`，08:00:00 开始、08:02:47 结束，总耗时 167.525 秒，安全
  错误码为 `pushplus.access_key_rejected`。本次不计入所需连续五次成功，因此 JAI-028 仍为 0/5。
- 采集第 1、2 次尝试因来源 5 的临时 HTTP 重试耗尽而失败，第 3 次恢复并完成全部五个来源。
  抽取/校验、匹配与日报随后成功，产出 11 条岗位公告、1 个岗位、10 条匹配结果和不可变日报快照
  `4`；仅投递阶段在 PushPlus 受理一次提交并返回持久 provider 身份后失败。
- 投递 `2` 与其唯一尝试 `2` 继续保守保存为 `unknown`，错误码为
  `pushplus.access_key_rejected`；provider 身份、安全错误和完整历史均保留。未发生重试、重发、人工
  发送、状态修正或重复尝试。APScheduler 的 callable 执行完成日志不构成验收成功，持久化流水线与
  投递状态才是权威证据。
- 按预先批准的 A-009 异常规则，已停止唯一 scheduler，并核验其为 `Exited (143)`；`api` 与 `db`
  保持健康。停止后的只读复核仍为同一条失败运行和同一条不明确尝试，没有数据改写。任何密钥修复、
  scheduler 重启、新观测窗口或真实 provider 操作前，均需负责人重新明确决定。
- 09:30 审计时 feature 分支末端仍为 `0614e21`，不存在 08:15 自动化生成的证据提交。本次人工审计
  只记录事实，不推测 heartbeat 是未启动还是在提交前失败。
- 负责人随后报告已把 Docker 公网出口 IPv4 加入 PushPlus，并把开发测试策略调整为允许有记录的补跑/
  重发。一次仅凭据的真实核验使用自动删除的 scheduler 服务临时容器，只调用 `getAccessKey`；结果为
  HTTP 200、provider 业务码 `403` 且没有 AccessKey，因此没有调用 `/send`，也没有数据库写入、补跑
  或重发。临时容器已自动删除。
- 随后发现正式 scheduler 从 `Asia/Shanghai` 12:12 起意外运行，重启次数为零；已按 A-009 于 12:42
  再次停止。日志只有 scheduler 启动，数据库仍是运行 `4` 的原七条阶段行及投递 `2` 的唯一尝试 `2`；
  期间没有新增流水线、阶段、投递或通知尝试行。

### 2026-09-26 — A-011 G1 可审计开发恢复实现完成

- 负责人只批准 G1：迁移与应用代码、显式 CLI，以及仅针对名称以 `_test` 结尾数据库的合成验证；
  不包括业务迁移、PushPlus 访问、凭据检查、真实补跑/重发或 scheduler 启动。
- 新增只添加结构的迁移 `0011_delivery_operator_audit`、ORM/契约以及只追加的
  `notification_delivery_operator_events` 台账。每次重发授权会在创建新尝试前记录有界原因和重复
  风险确认；开始与安全完成事件使用同一 UUID。受限外键、操作/事件唯一身份和 PostgreSQL 触发器
  会保留既有尝试，并拒绝事件更新/删除。
- 新增仅限 `development` 的 `jobagent-delivery resend`，目标为一个明确投递分段。它只接受既有终态
  `failed`/`unknown`/`interrupted` 分段，只提交一次，复用现有 advisory lock 和确定性消息身份，
  并追加终态结果。普通 `send` 对 `unknown` 的生产默认禁止语义保持不变。
- 既有流水线补跑继续通过逻辑运行、`trigger=makeup` 和编号阶段尝试形成显式只追加记录；G1 没有
  进行任何运行态调用。完整门禁通过：Ruff format 检查 266 个文件，Ruff lint 通过，Mypy 检查
  176 个源文件，358 项测试全部通过且无跳过，覆盖率为 85.57%。`_test` 场景证明尝试 1 保持不变、
  尝试 2 通过合成 provider 成功、恰好追加三个有序操作事件、成功后第二次重发被拒绝，且数据库
...
- 2026-09-25 运行环境复查：Docker 客户端 29.6.2 已安装，但 `npipe:////./pipe/docker_engine` 不存在；A-006 台账核验与 `M-003` 容器构建验证仍等待负责人手动恢复 Docker 守护进程。
- 2026-09-25 恢复后的 A-006 审计：Compose 恰有一个 `db`、`api` 和 `scheduler`；Alembic 为 `0009_pipeline_scheduling`；一个固定作业下次为 2026-09-26 08:00；两条运行/八个阶段均成功；2026-09-16 至 2026-09-25 十天无运行行；启动日志没有显式 misfire 事件；本地无 `node:24-alpine`。
- A-006 关闭核验：只有 `scheduler` 已停止（`Exited (143)`）；`db`/`api` 保持健康。停止后以只读方式确认 APScheduler 作业行保留，两条运行/八条阶段台账未变化。
- M-001/M-002 核验：两个必需变量名各出现一次且值非空；`.env` 保持被 Git 忽略；未输出任何密钥值。M-003 核验：`docker compose build api` 成功，镜像包含前端入口/JS/CSS，既有服务容器 ID 未变化，scheduler 保持停止。

## 5. 下一步

1. 保持 scheduler 停止并继续修正 PushPlus 安全 IP，直到另行批准的仅鉴权检查成功取得 AccessKey；
   最近一次真实检查仍为 `403`。
2. 业务 Alembic 保持 `0010`；未经下一次明确批准，不得把 `0011` 应用于业务数据、针对业务数据
   运行新 CLI、访问 PushPlus、执行补跑或重启 scheduler。
3. 鉴权与审计路径分别获准用于真实环境后，再批准任何真实重发/补跑及新的无人值守观测窗口；JAI-028 关闭前不得
   启动 JAI-051 或 JAI-029。

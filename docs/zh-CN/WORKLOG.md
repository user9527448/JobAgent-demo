...
2. 修改双语开发计划/Backlog 或增加前端依赖前保持 `A-002` 待审批；如获批，在 JAI-027 集成后通过专用 JAI-050 分支把 JAI-050/JAI-051 与调整后的 JAI-031 范围写入四份计划文件，再准备供 `A-003` 选择的三个 U2 视觉方向与成对 DESIGN.md 草案。

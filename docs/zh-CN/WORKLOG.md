...
> 最后更新：2026-09-23
>
> 当前分支：`feature/jai-052-spreadsheet-source-contract`
...
| JAI-050 | 实现检查点已封存；未完成、未合并 | `feature/jai-050-production-ui-foundation` / `96fe798...` | 正式页面壳保留给 JAI-053；容器验证与按序集成仍待完成 |
| JAI-052 | G1 后撤回；未完成且从未实现 | `feature/jai-052-spreadsheet-source-contract` | 仅保留远端审计历史；已从产品路线取消，不得再次访问共享表格 |
...
负责人明确确认飞书多维表格是共享资源，绝不能被修改。因此 JOBAGENT 不存在表格写回模式：应用代码、
测试、运维命令、恢复流程及后续闸门均不得修改单元格、视图、筛选、排序、评论、共享、权限或其他任何
来源状态。该限制独立于 Provider 当前权限；即使未来取得更高权限凭据，也继续生效。

### D-041 撤回表格加速路线并恢复原计划

2026-09-23，负责人确认共享资源不允许下载、导出或复制后，明确撤回 JAI-052/JAI-053。已发布审计
提交作为不可改写历史保留，但不合入产品主线；不得再次访问来源、请求凭据，也不得继续实现该路线
的表格 Reader、同步、Agent 或问答页面。活动关键路径恢复为
JAI-027 → JAI-050 → JAI-028 → JAI-051 → JAI-029；既有 G5、`M-003`、U3、五次运行、发布和按序
集成闸门均保持不变。
...
### 2026-09-23 — 路线调整前检查点与 JAI-052 表格 Agent 路线启动

- 负责人明确提出增量路线调整：完整保留已完成的爬虫驱动进度，先把稍后提供的每日更新 Excel 工作簿作为 Agent 信息源，交付可用的只读 Agent 和正式问答/筛选页面，再恢复爬虫驱动主线。该调整登记为有边界的加速轨，不是替换架构。
- 修改计划前核验当前分支 `feature/jai-050-production-ui-foundation` 为 `96fe7984d0a46e5f2c94b2bddebee35adcdf1377`，工作区干净，仓库本地作者为 `user9527448 <2537759248@qq.com>`，HTTPS origin 未改变且不存在持久 Git HTTP/HTTPS 代理。`develop`/`origin/develop` 仍为 `a9e9b643...`；没有合并或改写历史。
- 已普通推送封存的 JAI-050 分支 `96fe798...` 和带注释标签 `checkpoint/pre-spreadsheet-agent-pivot-2026-09-23`；远端剥离核验精确指向 `96fe798...`。已普通推送封存的 JAI-027 分支 `ff423f1...`；首次尝试遇到 GitHub 443 超时，只有重试命令临时使用 `http://127.0.0.1:7892` 代理，没有持久化代理或修改远程地址。该检查点不是发布标签，两个 Issue 均未据此视为完成。
- 2026-09-23 只读检查时 Docker Desktop Linux engine 不可用，因此无法读取 Alembic、APScheduler、`pipeline_runs`、`pipeline_stage_runs` 及 2026-09-16 至 2026-09-23 的调度证据。`A-006` 现为已过期且未决；旧的运行中/下一时刻结论已经失效，不推断成功、失败、misfire 或补跑。
- 已从核验后的检查点创建独立分支 `feature/jai-052-spreadsheet-source-contract`，新增成对 `SPREADSHEET_SOURCE.md`、开发计划/Backlog/人工队列更新及索引入口。获批路径为 JAI-052 → JAI-032 只读服务 → JAI-033 只读工具 → JAI-034 单 Agent/评测 → JAI-053 问答/筛选页面；封存的 JAI-027 → JAI-050 → JAI-028 → JAI-051 → JAI-029 只有在表格 Agent 验收且负责人确认后才恢复。
- 文档核验通过：计划、Backlog、WORKLOG、人工队列和表格契约的中英文标题数分别为 47/47、76/76、87/87、11/11、9/9；两份 Backlog 均有 59 个顺序一致的 Issue 标题；本次修改 Markdown 的相对链接全部有效；`git diff --check` 通过。
- 当前尚未提供工作簿链接。没有访问工作簿、猜测字段、处理凭据、执行数据库迁移/写入、启动 Docker、改变 scheduler、访问线上来源/provider、安装依赖或实现产品代码。`M-004/G1` 只要求链接和只读授权；准确映射与身份规则仍必须经 `A-007/G2` 批准后才能实现。

### 2026-09-23 — JAI-052 飞书来源 G1 只读审计完成

- 负责人提供飞书多维表格共享链接、授权只读检查，并明确禁止任何直接修改。`M-004` 已完成；D-040 把禁止写入/写回设为独立于未来 Provider 权限的永久边界。
- 匿名共享证据允许查看/预览，并拒绝编辑、评论、导出、打印、元数据/协作者管理、复制及操作历史访问。准确共享 URL、内部对象标识、查询 Token 与原始行均未写入 Git、日志、固定样本、文件或数据库。
- 主表 `❤秋招+春招汇总表` 报告 10,082 条记录、十个视图和 18 个字段。辅助表分别报告 200、321 和 12 条记录；没有检查其记录内容与完整 Schema，建议首期来源排除它们。
- 初始响应只暴露一个 2,000 条记录的值窗口。结构分析发现富文本与标量/数组多选混合表示、URL 重复组，而该窗口没有 `(公司名称, 招聘岗位, 投递链接)` 完全重复。这些明确只是样本窗口观察，不是总体结论。
- 全部 10,082 条记录的元数据显示 2026-09-18 至 09-23 每天都有修改。这证实来源持续逐日变化，但不能证明仅追加，也不能保证未来刷新时刻。
- G2 建议使用 provider/workbook/table/record 身份，复用现有 source/crawl-run/raw-document/job-post/job-position 领域，增加与 Provider 无关的只读快照 Reader，以 `feishu-rollup-v1` 对规范化规则版本化，把缺失记录记为 `not_observed`，且只允许合成固定样本与 `_test` 数据库测试。若未来可获得负责人授权的只读权限，优先使用飞书正式 OpenAPI；周期性公开共享获取仍受 G4 审核保护。
- 浏览器自动化与静态网页访问未能稳定读取共享应用页面；一次有界公开只读 HTTP 响应成功，结构数据只在内存解码，没有创建导出或下载。三个探索性 PowerShell 解析命令在执行前出现语法错误，修正后继续；它们未改变外部或仓库状态。
- 没有应用代码、依赖、迁移、业务数据导入、scheduler 变更、Agent/UI 实现、凭据处理或来源写入。下一闸门为 `A-007/G2`。
- 文档检查通过：五组成对修改文档标题数分别为 47/47、76/76、89/89、11/11 和 13/13；两份 Backlog 均有 53 个顺序一致的 Issue 标题；十份变更 Markdown 的相对链接全部有效；仓库文本中不存在共享 URL/查询标识；`git diff --check` 通过。

### 2026-09-23 — 表格加速方案撤回并恢复原计划

- 负责人确认没有下载、导出或复制权限后，要求遗忘表格数据源方案并继续原设计与计划。D-041 在不删除或改写既有事实的前提下登记该决定。
- JAI-052 只有文档/审计，标记为撤回而非完成；JAI-053 从未启动并一并撤回。`A-007/G2` 已取消，不再继续凭据申请、联系所有者、API/镜像尝试、导入器、迁移、同步、Agent 或页面工作。
- 已恢复 JAI-032～JAI-035 的原依赖和范围。活动 MVP 路线为 JAI-027 → JAI-050 → JAI-028 → JAI-051 → JAI-029；JAI-050 仍是唯一获批堆叠例外，且不得先于 JAI-027 集成。
- JAI-052 分支及普通推送历史保留为未实施方案的证据。共享链接从未提交，后续不再访问来源。本撤回记录没有修改应用代码、依赖、数据库、Docker、scheduler、provider、业务数据或原 feature 分支。
- 撤回文档检查通过：五组成对修改文档标题数分别为 47/47、76/76、91/91、11/11 和 14/14；两份 Backlog 均保留 53 个顺序一致的 Issue 标题；十份变更 Markdown 的相对链接全部有效；仓库文本中不存在共享链接/查询标识；`git diff --check` 通过。
...
1. 提交并普通推送 JAI-052 撤回记录，核验本地/跟踪/GitHub 一致，再返回未改变的 JAI-050 远端检查点，不合并 JAI-052。
2. Docker 已报告可用后，执行已过期 `A-006` 的 Compose/数据库只读审计；不得推断或执行补跑、迁移、投递或线上来源操作。
3. 继续原人工闸门：JAI-027 等待已延期的 `M-001`、`M-002` 与 `A-001/G5`；JAI-050 等待 `M-003` 容器验证。不得提前启动 JAI-028/JAI-051/JAI-029。

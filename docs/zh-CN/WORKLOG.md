...
2. 修改双语开发计划/Backlog 或增加前端依赖前保持 `A-002` 待审批；`A-003` 已选择方案 1“晨间简报”，但不提前解锁实现。如 `A-002` 获批，在 JAI-027 集成后通过专用 JAI-050 分支把 JAI-050/JAI-051 与调整后的 JAI-031 范围写入四份计划文件，并把已选方向固化为成对 DESIGN.md。

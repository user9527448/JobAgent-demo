...
1. 完成最终双语文档与差异检查，提交范围内 G1～G3 实施并普通推送 feature 分支。
2. 核验本地 feature HEAD、跟踪引用和 GitHub `ls-remote`，记录最终身份且不改写已发布历史。
3. 将 G4 业务库迁移/真实 scheduler 启用与 JAI-026 安全合并作为明确的负责人审核决定；在批准前不得执行。

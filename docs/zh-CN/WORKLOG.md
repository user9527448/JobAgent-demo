...
| JAI-028 | 第一次计划试运行因投递结果不明确而失败，当前暂停且为 0/5；已按 A-009 停止 scheduler | `feature/jai-028-e2e-unattended-trials` | 2026-09-26 运行 `4` 为 `failed`；投递 `2` 与尝试 `2` 为 `unknown`；不补跑、不重发 |
...
1. 保持 scheduler 停止，并原样保留运行 `4`、日报快照 `4`、投递 `2` 与尝试 `2`；不得补跑、
   重发或重新解释 2026-09-26 结果。
2. 负责人需在本地核对 PushPlus 用户 token/OpenAPI secret 配对及当前公网出口 IP 白名单，不得披露
   具体值。任何修复验证或 scheduler 重启均需重新明确批准，并登记新的观测窗口。
3. 在继续依赖无人值守核验前，先诊断 08:15 自动化为何没有生成 feature 分支证据；JAI-028 关闭前
   不得启动 JAI-051 或 JAI-029。

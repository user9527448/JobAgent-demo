...
> 最后更新：2026-09-25
>
> 当前分支：`feature/jai-050-production-ui-foundation`
...
| JAI-028 | 运行中；唯一已核验 scheduler 运行，`A-009/jai-028` heartbeat 已启用 | `feature/jai-028-e2e-unattended-trials` | 下一时刻 2026-09-26 08:00；每天 08:15 核验至 9 月 30 日，共五次；不补跑 |
...
1. 按 `A-007` 只使用已验证镜像重建 `api`，期间保持 scheduler 停止；核验健康状态、`/app/` 和容器
   身份。
2. 由自动化 `jai-028` 在每个计划时刻后核验唯一 scheduler 约束，并登记运行终态、阶段、日报、投递、可用率、完整率、
   解析成功率、耗时和重复证据。
3. 仅在数据库证据产生后逐次登记五次真实自动运行；JAI-028 关闭前不得启动 JAI-051 或 JAI-029。

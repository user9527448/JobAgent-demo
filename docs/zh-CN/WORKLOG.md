...
### D-034 历史评审集缺失的拟议恢复路径——必须审批

本方案尚未成为已批准的实施决策。推荐路径保留 JAI-025 原定的历史/人工评审目标，不把合成试运行集直接声明为充分验收。项目负责人批准后，以空的本地 `jobagent` 数据库作为受控证据库：从 Alembic `0003_attachment_storage` 升级到仓库 head；在事务中只初始化网站库里已经标记为 `active` 且 `enabled` 的五个来源；在任何持久化线上运行前，为现有手工采集路径增加向后兼容的可选详情条数上限。采集仅访问公开数据，并发为 1、请求间隔至少一秒、不得绕过登录/验证码/访问控制；详情尝试总数不超过 60，至少三个来源提供样本，单一来源进入评审的候选不超过 30 条。
...
1. 等待项目负责人明确批准 D-034/G1；批准前不得修改本地数据库、来源运行时记录或有界采集代码。
2. 若获批准，按顺序执行 G1～G5；每次推进前都在两份 WORKLOG 记录输入、有界动作、结果、偏离和审批。
3. JAI-026 调度、JAI-027 通知、JAI-030 维护 API、OCR/JAI-B01、JAI-049 与 JAI-048 均保持在当前 Issue 范围外。

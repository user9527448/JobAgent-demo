...
| JAI-028 | 运行中；`/app/` 已部署，唯一已核验 scheduler 按 `A-008` 运行 | `feature/jai-028-e2e-unattended-trials` | 下一时刻 2026-09-26 08:00；只按数据库证据计数五次自动运行；不补跑 |
...
1. 按 `A-007` 只使用已验证镜像重建 `api`，期间保持 scheduler 停止；核验健康状态、`/app/` 和容器
   身份。
2. 每个计划时刻及其后核验唯一 scheduler 约束，并登记运行终态、阶段、日报、投递、可用率、完整率、
   解析成功率、耗时和重复证据。

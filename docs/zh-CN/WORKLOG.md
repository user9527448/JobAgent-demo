...
| JAI-052 | G1 只读审计完成；等待 G2 审批 | `feature/jai-052-spreadsheet-source-contract` | 飞书主岗位表证据已登记；永久禁止来源写回；尚未开始实现 |
...
1. 取得负责人对已登记来源范围、身份、统一领域复用、规范化、缺失行语义和仅离线实现边界的明确 `A-007/G2` 决定。
2. 如获批准，只实现与 Provider 无关的只读快照 Reader、`feishu-rollup-v1` 确定性映射、合成固定样本及 `_test` 数据库测试；不得启动迁移、真实周期获取、业务导入、Agent、页面或任何飞书写入。
3. Docker 再次由负责人手动恢复后，先执行已过期 `A-006` 的 Compose/数据库只读审计，再决定 scheduler 状态；不授权补跑。继续延期 `M-001`、`M-002`、`A-001/G5` 和 `M-003`，不修改封存的 JAI-027/JAI-050 分支。

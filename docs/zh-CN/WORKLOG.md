...
| JAI-025 | 进行中；G1 已通过，等待负责人审批 G2 | `feature/jai-025-top-20-quality-review` | 有界采集工具与受控证据库已就绪；尚未持久化任何线上详情 |
...
1. 等待项目负责人明确批准或修改来源 ID 1～5 的 G2 配额 `3/0/2/5/0`。在此之前，不得发起或持久化任何详情请求。
2. 若 G2 获批，只执行这 10 条有界详情采集，再运行既有确定性解析/抽取/证据路径并报告不同岗位数量；若少于 50 条则停下等待新决策，不得自动增加来源或突破获批配额。
3. G3～G5 仍必须分别提供证据并取得审批；每个闸门的输入、有界动作、结果、偏离和审批都写入两份 WORKLOG。
4. JAI-026 调度、JAI-027 通知、JAI-030 维护 API、OCR/JAI-B01、JAI-049 与 JAI-048 均保持在当前 Issue 范围外。

...
| `A-007` | 已批准：2026-09-25 | 部署 JAI-050 API 镜像，并为 JAI-028 五次无人值守验收启动唯一 scheduler | JAI-050 `/app/` 运行态与 JAI-028 真实验收 |

`A-007` 只授权 JAI-028 正常计划时刻：运行唯一 scheduler、访问已批准的公开来源、写入相应业务数据，
并执行流水线内置的幂等 PushPlus 投递。它不授权补跑、人工发送/重发、scheduler 扩容、JAI-051 或
JAI-029 发布工作。JAI-027 早先的一次性真实通知额度仍已消耗，不在本次复用。

...
| ID | 状态 | 负责人操作 | 解锁事项 |
|---|---|---|---|
| `M-001` | 已完成并安全验证 | PushPlus 账户与 OpenAPI 设置已准备 | JAI-027 G5 凭据校验 |
| `M-002` | 已完成并安全验证 | 被忽略的本地 `.env` 包含两项非空 PushPlus 密钥 | JAI-027 G5 受控真实测试 |
| `A-001` | 可明确审批 | 明确批准 JAI-027 G5 | 业务迁移 `0010` 与一次指定真实测试 |

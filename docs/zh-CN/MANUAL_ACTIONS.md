...
| `A-009` | 已批准：2026-09-25 | 授权五日任务自动逐次审计、异常时停止 scheduler，并提交/推送双语证据 | 无人值守日常核验与持久 JAI-028 证据 |
...
创建五日核验 heartbeat 的尝试在创建前被拒绝，因为负责人尚未单独授权周期读取台账、异常时停止
scheduler、修改成对文档及普通 Git 提交/推送。当前尚无该自动化，scheduler 未被改变；要让每日
核验本身也无人值守，必须批准 `A-009`。

负责人已明确批准 2026-09-26 至 2026-09-30 的 `A-009`。每天计划运行后，任务可以读取 Docker 与
业务台账；成功时可更新成对 WORKLOG/必要计划状态、执行文档检查，并在 JAI-028 feature 分支普通
提交/推送。发现失败、重复、非终态超时、投递不明确、多个 scheduler 或镜像异常时，必须立即停止
scheduler 并通知负责人，禁止补跑或重发。第五次成功后必须停止 scheduler、执行 JAI-028 最终门禁
并推送关闭证据，但不得合并 `develop` 或启动后续 Issue。

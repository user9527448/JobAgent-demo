...
| `A-010` | 等待负责人决定 | 在本地核对 PushPlus OpenAPI 凭据/IP 白名单，并批准任何修复验证、scheduler 重启和替代观测窗口 | 仅在安全排除阻塞后恢复 JAI-028 |
...
负责人应在本地核对以下事项且不得披露具体值：`.env` 使用的是 PushPlus 用户 token 而非消息 token；
OpenAPI `secretKey` 与“开发设置”中的值仍一致；本机当前公网出口 IP 已列入 PushPlus 安全 IP 白名单。
不得手工创建或粘贴 AccessKey，JOBAGENT 会在内存中获取。本地核对不授权测试消息、重发、scheduler
重启或替代试运行窗口；只读诊断审阅后，这些动作仍需负责人另行明确批准 A-010。

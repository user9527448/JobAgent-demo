...
| `M-004` | 历史已完成；路线已撤回 | 曾提供飞书多维表格共享链接供已完成的 G1 审计 | 仅作历史证据 |
| `A-007` | 负责人已取消：2026-09-23 | 不再请求表格实现审批 | 无；恢复原计划 |
...
## M-004/A-007 — 已撤回的表格数据源路线

`M-004` 过去只授权现已完成的 G1 只读检查，从未授权数据库迁移、生产导入、周期获取、表格写回、
Agent 实现、爬虫改动、凭据处理或绕过访问控制。该路线现已关闭，不再要求负责人执行任何后续操作。

`M-004/G1` 已于 2026-09-23 完成。公开共享证据证明仅有查看/预览权限、主岗位表含 10,082 条
记录和 18 个字段，并存在逐日修改元数据；没有改变来源状态、数据库、文件或 scheduler。准确共享
URL 和响应数据均不进入 Git。从 JOBAGENT 视角，共享来源永久不可写：后续任何闸门都不得授权修改
单元格、视图、筛选、排序、评论、共享或权限。

原建议 `A-007/G2` 审批文本仅作为历史证据保留，不得再使用：

```text
批准 JAI-052 A-007/G2：首期只接入主岗位表；使用 provider/workbook/table/record 身份；复用现有
source、crawl-run、raw-document、job-post 和 job-position 领域；只实现与 Provider 无关的只读快照
Reader、确定性的 feishu-rollup-v1 映射、合成固定样本及 _test 数据库测试；缺失记录只记为
not_observed 且不删除；永不写回飞书。真实周期获取、业务数据库变更、Agent/UI 工作及任何来源写入
均未获授权。
```

2026-09-23，负责人确认共享资源不允许下载、导出或复制后撤回该路线。`A-007` 已取消；不再请求或
授权凭据、API 权限、镜像、导入器、数据库迁移、同步、Agent 或页面，也不得再次访问该共享表格。
项目恢复 JAI-027 → JAI-050 → JAI-028 → JAI-051 → JAI-029 原关键路径。

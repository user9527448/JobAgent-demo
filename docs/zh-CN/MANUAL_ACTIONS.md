...
| `A-006` | 截止时刻已过；2026-09-25 运行环境不可用 | Docker 恢复后依据只读台账决定是否继续启用每日计划业务运行 | 在任何后续 scheduler 变更前明确运行状态 |
...
2026-09-16 的决定截止时刻已过，但没有登记负责人决定。2026-09-25 只读检查确认 Docker 客户端存在，
守护进程管道不可用，因此无法读取 Compose、Alembic、APScheduler 和流水线台账。2026-09-16 至
2026-09-25 没有可用证据；此前“正在运行/下一时刻”的状态已经过期，不能据此推断成功、失败或 misfire。

Docker 恢复后，先由 Agent 只读核验运行环境和台账，再由负责人决定继续普通每日业务运行，或批准只
停止 scheduler。本事项不授权补跑、迁移、投递或来源命令。

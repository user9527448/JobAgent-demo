...
| `A-008` | 第一次运行后暂停：2026-09-26 | 授权最多五次 JAI-028 自动运行生成的日报/岗位内容通过 PushPlus 外发 | 第一次运行因投递结果不明确而失败；当前计数 0/5 |
| `A-009` | 已执行异常动作：2026-09-26 | 授权五日任务自动逐次审计、异常时停止 scheduler，并提交/推送双语证据 | scheduler 已停止；失败/不明确证据已保留 |
| `A-010` | 复测失败：provider `403` | 在本地核对 PushPlus OpenAPI 凭据/IP 白名单，并批准任何修复验证、scheduler 重启和替代观测窗口 | 安全 IP 配置仍阻止获取 AccessKey |
| `A-011` | G1 完成；G2 返回 provider `403`；G2.1 出口 IP 查询完成 | 把另行报告的当前容器公网出口 IPv4 与 PushPlus 安全 IP 条目逐字核对，并保存任何修正 | 鉴权仍阻塞；下一次仅鉴权检查与全部真实恢复操作均需新审批 |
...
线程 heartbeat 已成功创建为自动化 `jai-028`，按 `Asia/Shanghai` 每天 08:15 执行，共五次。创建时已
经过 2026-09-25 的核验时刻，因此五次覆盖 2026-09-26 至 2026-09-30；读取已保存自动化确认其仍为
活动状态。前两次创建尝试均未产生自动化或运行态变化：一次是本地调用引号语法错误，另一次因立即
创建不能包含显式 DTSTART 而被拒绝。

第一次计划试运行于 2026-09-26 08:00 恰好执行一次。采集第 3 次尝试恢复，抽取、匹配和日报生成
成功，但投递核对返回 `pushplus.access_key_rejected`。因此流水线运行 `4` 为 `failed`，投递 `2`
及其唯一尝试 `2` 保持 `unknown`，持久 provider 身份仍原样保留。已按 A-009 停止唯一 scheduler，
并核验其为 `Exited (143)`；`api` 与 `db` 保持健康。没有补跑、重发、状态修正或重复尝试，验收仍为
0/5。09:30 审计时不存在 08:15 自动化生成的证据提交，因此继续依赖无人值守核验前也必须诊断该项。

## A-010 — 恢复 JAI-028 前排除 PushPlus OpenAPI 阻塞

负责人已确认使用用户 token、OpenAPI 已启用、`secretKey` 一致，并报告已加入 Docker 公网出口 IPv4。
随后一次真实但仅凭据的检查只调用 `getAccessKey`，返回 HTTP 200、provider 业务码 `403` 且无
AccessKey；没有调用 `/send` 或写数据库，临时容器已自动删除。另行发现正式 scheduler 自 12:12 起
运行，已于 12:42 再次停止，台账没有新增行。因此必须继续修正并保存 PushPlus 安全 IP 配置，之后再
另行批准下一次检查。

## A-011 — 新增可审计的仅开发环境补跑/重发路径

负责人于 2026-09-26 批准 G1：实现只新增结构的迁移 `0011`、只追加操作事件台账和显式的仅开发
环境重发 CLI，并且只在 `_test` 数据库配合合成 provider 验证。实现通过新增尝试而非改写原
`unknown` 尝试，在 provider 交互前写入授权与开始事件，并追加安全完成结果；生产环境继续默认
禁止。既有补跑仍为显式命令，并由不可变运行身份、`trigger=makeup` 和编号阶段行审计。

G1 不授权把 `0011` 应用于业务库、访问 PushPlus、执行真实重发或补跑、检查真实凭据或重启
scheduler。这些影响必须等鉴权已确认可用后，通过下一项独立审批闸门。

随后负责人批准 G2，只执行一次仅鉴权的 `getAccessKey` 复测。一次性容器得到 HTTP 200，但 provider
业务码仍为 `403`，没有取得 AccessKey；容器已自动删除，正式 scheduler 保持 `Exited (143)`。没有
调用 `/send`、写业务库、执行迁移或补跑，也没有启动 scheduler。已按要求停止；在修正 PushPlus
安全 IP/凭据配置并另行批准下一次仅鉴权复测前，不开放任何真实恢复闸门。

负责人批准 G2.1，由一次性 scheduler 服务容器只查询一次公网出口 IPv4。查询成功，具体值已直接
报告负责人用于人工核对；仓库历史不得保存个人网络元数据，因此文档有意不记录该值。一次性容器已
自动删除，`db`/`api` 保持健康，正式 scheduler 仍为 `Exited (143)`。G2.1 没有访问 PushPlus 或
业务接口，也不解锁下一次鉴权复测或任何恢复操作。

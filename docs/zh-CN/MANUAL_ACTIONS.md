...
| `A-011` | 等待负责人审批 | 批准迁移 `0011`、只追加的开发重发授权及另行记录的第一次真实重发/补跑 | 在不削弱生产幂等的前提下实施 D-041 |
...
负责人已原则批准开发测试可以执行有记录的补跑与重发。拟议实现包括只新增结构的迁移 `0011`、
只追加的操作授权台账、要求有界原因和重复风险确认的显式开发环境命令、通过新增投递尝试而非改写
原 `unknown` 尝试，以及生产环境继续默认禁止。A-011 必须明确批准该数据模型及第一次真实重发；
真实操作只能在 AccessKey 鉴权成功后执行。

...
`post_form_query()` 把同一套控制扩展到“HTTP 契约采用表单 POST、但业务语义已经核验为公开只读查询”的端点。它不是通用 POST 客户端：Adapter 不得将其用于登录、注册、投递或其他状态变更。只读表单查询使用与 GET 相同的限速、重试分类和安全日志，但不使用条件缓存校验值。

...
开发补跑继续使用显式命令，并已经由 `pipeline_runs.trigger=makeup` 与只追加的编号
`pipeline_stage_runs` 审计；它不会删除失败的计划运行或阶段。迁移 `0011` 只为风险更高的 provider
重发边界新增独立操作授权台账，因为这里必须保留原有不明确外部身份。G1 只在 `_test` 上验证两类
恢复台账，不授权实际执行 `makeup`、重启 scheduler 或访问真实 provider。

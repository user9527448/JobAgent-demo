...
- 央国企招聘：优先稳定的官方聚合公告或代表性央企公开招聘栏目；持续不可达的来源按稳定性闸门停用并保留诊断。
...
| 央国企 | 全国 | 国务院国资委 | [公开招聘栏目](https://www.sasac.gov.cn/n2588035/n2588325/n2588350/index.html) | 已阻塞 | JAI-011 首个 Adapter；连续两日公网/TLS 不可达，已停用但保留历史契约与诊断 |
| 央国企 | 全国/江浙沪 | 国家电网 | [招聘平台](https://zhaopin.sgcc.com.cn/) | 待接入 | 动态门户，需先验证公开接口、服务条款和页面稳定性 |
| 央国企 | 全国/江浙沪 | 中国移动 | [招聘公告](https://job.10086.cn/personal/notice/) | 已启用 | JAI-021 稳定性替代来源；只读页面自身声明的同源静态 JSON 和公开详情，不访问账号或投递动作 |
...
“待接入”只表示已登记候选来源，不表示当前程序会访问它。机器配置要求此类来源为 `implementation_status = "planned"` 且 `enabled = false`；“已阻塞”使用 `blocked` 且同样不得启用。

现有 11 个官方候选站全部进入实施路线：JAI-011/JAI-021 当前启用 5 个，中国移动因稳定性替换提前吸收 JAI-041，国资委保留为 1 个阻塞来源，其余 5 个计划来源仍按 JAI-038～JAI-043 的未完成项逐站处理。动态门户无法在无需登录、无需验证码且条款允许的条件下稳定读取时，必须保持 `planned` 或改为 `blocked`，并优先寻找稳定公开公告入口；“全部纳入目标”不等于允许绕过限制强行启用。
...
2026-08-27，国务院国资委公开招聘栏目连续第二日以可重试 `PoolTimeout` 失败；用户普通浏览器也无法打开，`www` 公网 CDN 连接失败，官方 `wap` 入口证书过期。按 W6 稳定性闸门将其设为 `blocked`/停用。中国移动官方招聘公告页及其同源静态 JSON 列表/详情低频 GET 返回 200，并在 2026-08-27 与 2026-08-29 的有界运行中分别完成 3 条详情。由于 2026-08-28 没有观测，连续序列已于 2026-08-29 重新从第 1 日开始。不得把 JSON 下线时间猜成报名截止；纯图片正文只保留同源图片 URL，不下载、不做 OCR；也不访问注册、登录、简历或投递功能。

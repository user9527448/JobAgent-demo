...
2026-08-27，国务院国资委公开招聘栏目连续第二日以可重试 `PoolTimeout` 失败；用户普通浏览器也无法打开，`www` 公网 CDN 连接失败，官方 `wap` 入口证书过期。按 W6 稳定性闸门将其设为 `blocked`/停用。中国移动官方招聘公告页及其同源静态 JSON 列表/详情低频 GET 返回 200，替换后的最终有界运行完成 3 条详情并使全部 5 个活动来源成功，成为连续稳定性第 1 日；过程中也出现过一次列表 `PoolTimeout`，仍需继续观测。不得把 JSON 下线时间猜成报名截止；纯图片正文只保留同源图片 URL，不下载、不做 OCR；也不访问注册、登录、简历或投递功能。
